...
(function($, window, document, undefined) {
        ...
        $containers.each(function() {
            ...
            if (!apiScriptLoaded && !apiScriptLoading) {
                var apiArgs = {
                  ...
                  /*  signed_in: thisOptions.signed_in */ /* Deprecated */ 
                };
                if (thisOptions.key)
                    apiArgs.key = thisOptions.key;
                if (thisOptions.libraries)
                    apiArgs.libraries = thisOptions.libraries;
                if (thisOptions.language)
                    apiArgs.language = thisOptions.language;
                if (thisOptions.region)
                    apiArgs.region = thisOptions.region;
    ...
    $window.on("googleMapsScriptLoaded", function() {
        ...
        init()
    }).on("scroll", throttle(500, init)).on("resize", debounce(1e3, function() {
        windowHeight = $window.height();
        init()
    }));
